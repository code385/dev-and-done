...
export async function POST(request) {
  try {
    ...
    const fileName = `blogs/cover-images/${timestamp}_${sanitizedFileName}`;
    ...
    if (process.env.BLOB_READ_WRITE_TOKEN) {
      try {
        ...
        const uploadsDir = join(process.cwd(), 'public', 'uploads', 'blogs', 'cover-images');
        if (!existsSync(uploadsDir)) {
          await mkdir(uploadsDir, { recursive: true });
        }
        const filePath = join(uploadsDir, `${timestamp}_${sanitizedFileName}`);
        await writeFile(filePath, buffer);
        publicUrl = `/uploads/blogs/cover-images/${timestamp}_${sanitizedFileName}`;
      }
    } else {
